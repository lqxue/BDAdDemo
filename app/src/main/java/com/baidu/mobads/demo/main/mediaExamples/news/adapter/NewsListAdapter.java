...
public class NewsListAdapter extends RecyclerView.Adapter<FeedItemViewHolder> {
    ...
    public void onViewAttachedToWindow(@NonNull FeedItemViewHolder viewHolder) {
        ...
        if (viewHolder instanceof FeedAdViewHolder) {
            ...
            } else if (FeedParseHelper.isDownloadAd(nrAd)) {
                // 非视频广告，有下载信息则进行展示
                downloadButton = holder.mDownLoadInfoContainer.findViewById(R.id.native_download_button);
            }
            final CustomProgressButton fDownloadButton = downloadButton;
            /**
             * registerViewForInteraction()与BaiduNativeManager配套使用
             * 警告：调用该函数来发送展现，勿漏！
             */
            Log.e(TAG, "onViewAttachedToWindow: inter");
            nrAd.registerViewForInteraction(holder.itemView, new NativeResponse.AdInteractionListener() {
                @Override
                ...
                public void onADStatusChanged() {
                    ...
                    // 更新下载按钮的进度
                    if (fDownloadButton != null) {
                        fDownloadButton.updateStatus(nrAd);
                    }
                }
