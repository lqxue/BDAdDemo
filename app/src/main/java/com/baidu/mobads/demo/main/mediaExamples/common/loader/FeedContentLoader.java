 ...
 *
 * 初始化方法：{@link #getInstance(Context, int, int, IdIterator, LoadListener)}
 *
 * 【-------=====警告=====-------】
 * 此加载器是为了填充Demo的展示内容，使用了内容联盟
 * 提供的数据，媒体需要自行获取内容数据。
 */
public class FeedContentLoader<E> extends AbstractFeedLoader<E> {
    ...
    public static <T> FeedContentLoader<T> getInstance(Context context, int contentType,
                              int threshold, IdIterator iterator, LoadListener listener) {
        final FeedContentLoader<T> loader =
            ...
            public void run() {
                ...
                for (String content : contentList) {
                    try {
                        ...
                        Log.w(TAG, "loadData: JSON!!! " + bufferQueue.size());
                        FeedItem item = getItemFromJob(id, contentType, contentJob);
                        bufferQueue.offer((E) item);
